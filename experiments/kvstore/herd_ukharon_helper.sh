...
failover="$3" # `--failover` or no argument given
...
HERDSRV1_ARGS="-p 5 -m 5 -s 6 -m $UKHARON_MCGROUP -k $UKHARON_KERNELMCGROUP --access $access"
HERDSRV2_ARGS="-p 6 -m 5 -s 6 -m $UKHARON_MCGROUP -k $UKHARON_KERNELMCGROUP --access $access"
HERDCLI_ARGS="-p 7 -m 5 -s 6 -m $UKHARON_MCGROUP -k $UKHARON_KERNELMCGROUP --load $operation $failover"
...
if [[ -z $failover ]]; then
  ...
  sleep 5
  # Kill the primary
  ssh -o LogLevel=QUIET -t $(machine2ssh machine2) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine2)/deployment/herd-server-stop.sh"
