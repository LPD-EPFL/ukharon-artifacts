...
FAILOVER_ARGS="-p 6 -m $UKHARON_MCGROUP -k $UKHARON_KERNELMCGROUP -s $UKHARON_SYNCKILLERMCGROUP -w 3"
...
ssh -o LogLevel=QUIET -t $(machine2ssh machine2) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine2)/deployment/acceptor.sh binaries/membership_acceptor $ACCEPTOR3_ARGS"
sleep 3

# Caches (one with sync-killer)
ssh -o LogLevel=QUIET -t $(machine2ssh machine3) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine3)/deployment/cache.sh binaries/membership_cache $CACHE1_ARGS $heartbeat"
ssh -o LogLevel=QUIET -t $(machine2ssh machine3) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine3)/deployment/sync-killer.sh binaries/sync_killer cache $SYNCKILLER_ARGS"
...
ssh -o LogLevel=QUIET -t $(machine2ssh machine4) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine4)/deployment/member.sh binaries/membership_member $MEMBER_ARGS $heartbeat"
ssh -o LogLevel=QUIET -t $(machine2ssh machine4) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine4)/deployment/sync-killer.sh binaries/sync_killer member $SYNCKILLER_ARGS"
sleep 2

# Failover test
ssh -o LogLevel=QUIET -t $(machine2ssh machine7) "$ROOT_DIR/ukharon_experiment/$(machine2dir machine7)/deployment/failover.sh binaries/test_cache_coordinated_failures $FAILOVER_ARGS"
sleep 2
...
gather_results "$SCRIPT_DIR"/logs/failover_app_leasecache_coord/latency_${heartbeat_str}
# clear_processes
